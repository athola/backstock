"""Connexion-based Flask application with OpenAPI specification using factory pattern."""
...
def create_app(config_name: str | None = None) -> FlaskApp:
    """Create and configure the Connexion Flask application.

    Args:
        config_name: Configuration class name (e.g., 'DevelopmentConfig', 'ProductionConfig').
                    If None, uses APP_SETTINGS environment variable or defaults to DevelopmentConfig.

    Returns:
        Configured Connexion FlaskApp instance.

    Raises:
        RuntimeError: If OpenAPI specification cannot be loaded.
    """
    ...
    connexion_app = connexion.FlaskApp(
        __name__,
        specification_dir=str(root_dir),
    )
    ...
    # Configure Flask app
    flask_app.template_folder = str(root_dir / "templates")
    if config_name is None:
        config_name = os.environ.get("APP_SETTINGS", "src.pybackstock.config.DevelopmentConfig")
    ...
    import src.pybackstock.app as app_module  # noqa: PLC0415

    # Import models after db is created
    from src.pybackstock.models import Grocery  # noqa: PLC0415, F401
...
# Create default app instance for deployment (Gunicorn/WSGI servers)
