...
inventoryApp.config.from_object(
    ...
    if request.method == "POST":
        if "search-item" in request.form:
            ...
            try:
                ...
                error_type = (
                    "Unable to add item. Please double check your item parameters. "
                )
                errors = report_exception(ex, error_type, errors)
...
def report_exception(ex, error_type, errors):
    error_str = (
        error_type
        + str(ex)
        + " - Error on line no: {}".format(sys.exc_info()[-1].tb_lineno)
    )
...
def get_matching_items(search_column, search_item):
    ...
    if search_column == "last_sold":
        return Grocery.query.filter(
            func.to_char(Grocery.last_sold, "%YYYY-MM-DD%").ilike(
                search_term,
            )
        ).order_by(Grocery.id)
    return Grocery.query.filter(
        getattr(Grocery, search_column).ilike(
            search_term,
        )
    ).order_by(Grocery.id)
...
def add_item(item, errors, items):
    try:
        ...
        item_exists = (
            db.session.query(Grocery).filter(Grocery.id == item.id).first() is not None
        )
        if not item_exists:
            db.session.add(item)
            db.session.commit()
            ...
            errors.append(
                "Unable to add item to database. This item has already been added with ID: "
                + str(item.id)
            )
    except Exception as ex:
        db.session.rollback()
