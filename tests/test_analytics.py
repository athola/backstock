...
def test_stock_health_analytics_with_multiple_items(client: FlaskClient, app: Flask) -> None:
    ...
    with app.app_context():
        ...
        items = [
            Grocery(item_id=301, description="Out of Stock", last_sold=None, department="Test",
                   price="1.99", cost="0.99", unit="ea", x_for=1, shelf_life="7d",
                   quantity=0, reorder_point=10),
            Grocery(item_id=302, description="Low Stock", last_sold=None, department="Test",
                   price="2.99", cost="1.99", unit="ea", x_for=1, shelf_life="7d",
                   quantity=5, reorder_point=10),
            Grocery(item_id=303, description="Good Stock", last_sold=None, department="Test",
                   price="3.99", cost="2.99", unit="ea", x_for=1, shelf_life="7d",
                   quantity=25, reorder_point=10),
        ]
        for item in items:
            db.session.add(item)
        db.session.commit()
...
def test_inventory_value_calculation(client: FlaskClient, app: Flask) -> None:
    ...
    with app.app_context():
        ...
        items = [
            Grocery(item_id=401, description="Item 1", last_sold=None, department="Test",
                   price="10.00", cost="5.00", unit="ea", x_for=1, shelf_life="7d",
                   quantity=10, reorder_point=5),
            Grocery(item_id=402, description="Item 2", last_sold=None, department="Test",
                   price="20.00", cost="10.00", unit="ea", x_for=1, shelf_life="7d",
                   quantity=5, reorder_point=5),
        ]
        for item in items:
            db.session.add(item)
        db.session.commit()
...
def test_reorder_alerts_identification(client: FlaskClient, app: Flask) -> None:
    ...
    with app.app_context():
        ...
        items = [
            Grocery(item_id=501, description="Needs Reorder", last_sold=None, department="Test",
                   price="1.99", cost="0.99", unit="ea", x_for=1, shelf_life="7d",
                   quantity=5, reorder_point=10),
            Grocery(item_id=502, description="Stock OK", last_sold=None, department="Test",
                   price="2.99", cost="1.99", unit="ea", x_for=1, shelf_life="7d",
                   quantity=15, reorder_point=10),
        ]
        for item in items:
            db.session.add(item)
        db.session.commit()
...
def test_inventory_age_distribution(client: FlaskClient, app: Flask) -> None:
    ...
    with app.app_context():
        ...
        items = [
            Grocery(item_id=601, description="New Item", last_sold=None, department="Test",
                   price="1.99", cost="0.99", unit="ea", x_for=1, shelf_life="7d",
                   quantity=10, reorder_point=5, date_added=today),
            Grocery(item_id=602, description="Week Old", last_sold=None, department="Test",
                   price="2.99", cost="1.99", unit="ea", x_for=1, shelf_life="7d",
                   quantity=10, reorder_point=5, date_added=today - timedelta(days=7)),
            Grocery(item_id=603, description="Month Old", last_sold=None, department="Test",
                   price="3.99", cost="2.99", unit="ea", x_for=1, shelf_life="7d",
                   quantity=10, reorder_point=5, date_added=today - timedelta(days=30)),
        ]
        for item in items:
            db.session.add(item)
        db.session.commit()
...
def test_department_analytics(client: FlaskClient, app: Flask) -> None:
    ...
    with app.app_context():
        ...
        items = [
            Grocery(item_id=701, description="Produce Item", last_sold=None, department="Produce",
                   price="1.99", cost="0.99", unit="ea", x_for=1, shelf_life="7d",
                   quantity=10, reorder_point=5),
            Grocery(item_id=702, description="Dairy Item", last_sold=None, department="Dairy",
                   price="2.99", cost="1.99", unit="ea", x_for=1, shelf_life="7d",
                   quantity=15, reorder_point=5),
            Grocery(item_id=703, description="Another Produce", last_sold=None, department="Produce",
                   price="3.99", cost="2.99", unit="ea", x_for=1, shelf_life="7d",
                   quantity=20, reorder_point=5),
        ]
        for item in items:
            db.session.add(item)
        db.session.commit()
...
def test_price_range_analytics(client: FlaskClient, app: Flask) -> None:
    ...
    with app.app_context():
        ...
        items = [
            Grocery(item_id=801, description="Budget Item", last_sold=None, department="Test",
                   price="0.99", cost="0.49", unit="ea", x_for=1, shelf_life="7d",
                   quantity=10, reorder_point=5),
            Grocery(item_id=802, description="Mid-range Item", last_sold=None, department="Test",
                   price="5.99", cost="3.99", unit="ea", x_for=1, shelf_life="7d",
                   quantity=10, reorder_point=5),
            Grocery(item_id=803, description="Premium Item", last_sold=None, department="Test",
                   price="15.99", cost="10.99", unit="ea", x_for=1, shelf_life="7d",
                   quantity=10, reorder_point=5),
        ]
        for item in items:
            db.session.add(item)
...
def test_edge_case_all_items_out_of_stock(client: FlaskClient, app: Flask) -> None:
    ...
    with app.app_context():
        ...
        items = [
            Grocery(item_id=1101, description="OOS Item 1", last_sold=None, department="Test",
                   price="1.99", cost="0.99", unit="ea", x_for=1, shelf_life="7d",
                   quantity=0, reorder_point=10),
            Grocery(item_id=1102, description="OOS Item 2", last_sold=None, department="Test",
                   price="2.99", cost="1.99", unit="ea", x_for=1, shelf_life="7d",
                   quantity=0, reorder_point=10),
        ]
        for item in items:
